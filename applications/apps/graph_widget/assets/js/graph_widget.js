...
d3.graph = function() {
	...
	radius = 16,
	legend = new Set(),
	imgPath = null;// "/applications/apps/graph_widget/assets/images/";
...
/*            root.fixed = true;
	...
	var _nodes = [], _links = [];
		...
		if (!legend.has(nodes[i].type)) {
		    addLegend(legend.size, getName(nodes[i]), getColor(nodes[i]), getStrokeColor(nodes[i]), getImage(nodes[i]), getLabel(nodes[i]));
