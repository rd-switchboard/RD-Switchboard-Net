...
<br/>
<h3 id="search_video">How to search in Research Data Switchboard</h3>
<br/>
...
<p>The search within Research Data Switchboard will return records matching your specified search parameters (terms and filters). By default the returned records will be sorted by ‘Relevance’, where each record in a search result is given a ranking based on how closely a record matches the entered parameters. The search ranking algorithm used in Research Data Switchboard for ‘Relevance’ is complex and cannot easily be described. Below is a very rough guide for how the default ranking and matching is achieved.</p>
<ul>
    <li>Searches for a given word in Research Data Switchboard will generate results of any records which contain the entire word or the "stem" of the word (a stem is generated by a built-in Solr  filter, which breaks down search terms into their "word stems", so "fishing" will match "fish")</li>
